...
module Impl (KV: Mirage_kv_lwt.RO) = struct
  ...
  let refx = ref (Owl_base_dense_ndarray_s.empty [|nsamples; edge; edge; 1|])
  let refy = ref (Owl_base_dense_ndarray_s.empty [|nsamples; nclass|])
  ...
  let get_next_batch () =
    !refx, !refy (* FIXME: partial iteration *)
